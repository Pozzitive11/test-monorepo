...
@Component({
  selector: 'app-dc-other-docs-upload',
  templateUrl: './dc-other-docs-upload.component.html',
  standalone: true,
  imports: [
    NgIf,
    NgbProgressbar,
...
export class DcOtherDocsUploadComponent {
  ...
  get docTypes(): DocType[] { return this.dataService.otherDocTypes }

  get shown() { return this.dataService.shownDocUploadType.other }

  set shown(value) {
    this.dataService.shownDocUploadType.other = value
    if (value)
  ...
  uploadFiles() {
    if (this.selectedFiles) {
      ...
      for (let file of this.selectedFiles) {
        ...
        this.httpService.uploadOtherDocs(formData, this.INN, docType)
          .subscribe({
            next: (data) => {
